...
class GenerateLangTiddlers:
    ...
    def run(self, path, time=datetime.datetime.today().strftime('%Y%m%d%H%M')):
        ...
        for lang in langs:
            ...
            f.close()
        markup_tiddler = path + '/markup/MarkupPreHead.tiddler'
        markup_tiddler_div = markup_tiddler + '.div'        
        f = open(markup_tiddler, 'w')
        f.write('<!-- generated by generateLangTiddlers.py -->\n')
        f.write('<!-- DO NOT EDIT BY HAND -->\n')
        f.write('<!-- EDIT generateLangTiddlers.py INSTEAD -->\n')
        f.write('<!--{{{-->\n')
        f.write('<link rel="alternate" type="application/rss+xml" title="RSS" href="index.xml"/>\n')
        for lang in langs:
            f.write('<link href="l10n/jpoker-%s.json" lang="%s" rel="gettext"/>\n' % (lang, lang))
        f.write('<link href="css/jpoker.css" rel="stylesheet" type="text/css" />\n')
        f.write('<!--}}}-->')
        f.close()
        f = open(markup_tiddler_div, 'w')
        f.write('title="MarkupPreHead" modifier="script" created="%s" changecount="1"' % time)
        f.close()
